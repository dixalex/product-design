````markdown
# Handoff brief template (visual-design)

The OUTPUT artifact written at brief-write time. Lands at `<project-root>/docs/visual-design/<YYYY-MM-DD>-<slug>.md`. The brief has three audiences:

1. **Human review** — the designer / stakeholder / future-self reads the prose decisions and confirms intent
2. **Git versioning** — the brief is the design record; lives in the project repo
...
project: "<name>"
date: YYYY-MM-DD
domain: <crm | productivity | dashboard | mobile | dev-tool | design-tool | media | feed | other>     # inherited from structural brief
...
constraints_from_structural_brief: ["<surface — constraint>"]   # see § 2.5 of spec
---

# <Project> — visual brief

## Mood

<1 paragraph: the aesthetic commitment in the user's words. Cite the extraction signals from the structural brief that derived the mood (e.g., "JTBD's emotional verb 'work' + persona 'solo founder, keyboard-first' + scope rejection of 'manager dashboards' → terminal-adjacent, dense, calm").>
...
## Constraints from structural brief
